...
def convert(input_checkpoint_path, loaded_config, output_checkpoint_path):
    ...
    for layer_i in tqdm(range(get_key(loaded_config, "num-layers"))):
        ...
        for key in [
            ...
            # use_bias_in_attn_linear = False일 경우에는 이전 X
        ...
        # 어차피 만들어진 길이 대로 결정된다. 아래 두 state는 바로 생성해서 쓰면 동일하다.
        max_positions = hf_config.max_position_embeddings
        state_dict["attention.bias"] = torch.tril(
                torch.ones((max_positions, max_positions), dtype=torch.bool)
                ).view(1, 1, max_positions, max_positions)
        # use_bias_in_attn_linear = False일 경우에는 이전 X
        state_dict["attention.masked_bias"] = torch.tensor(-1e9)
